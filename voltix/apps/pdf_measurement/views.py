...
from .utils import save_pdf_temporarily, get_existing_pdf
    ...
    try:
        ...
        if existing_pdf_path:
            ...
            with open(existing_pdf_path, 'rb') as f:
                pdf = f.read()
            logger.info(f"Serving cached PDF for comparison ID {comparison_id}")
        ...
        logger.info(f"Requesting PDF generation from microservice with data: {comparison_data}")
        response = requests.post(microservice_url, json=comparison_data)

        # Si la solicitud al microservicio fue exitosa
        if response.status_code == 200:
            pdf_data = response.content
            logger.info(f"PDF successfully generated by microservice for comparison ID {comparison_id}")
            ...
            logger.error(f"Failed to generate PDF from microservice for comparison ID {comparison_id}")
            return JsonResponse({"error": "Failed to generate PDF."}, status=500)

    except Exception as e:
        logger.exception(f"Error generating report for comparison ID {comparison_id}: {str(e)}")
